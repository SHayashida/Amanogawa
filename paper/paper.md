...
For clustering analysis, Amanogawa computes the two-point correlation function ξ(r) using a catalogue-based DD/RR estimator against repeated Poisson reference sets [@peebleshauser1974; @peebles1980], alongside nearest-neighbour distance distributions [@clark1954]. The software also includes box-count scaling as a compact descriptive summary of multi-scale occupancy rather than as a formal estimator of fractal dimension, complementing ξ(r) by providing an intuitive, coarse-grained view of spatial structure. The software additionally supports magnitude-stratified clustering by pairing simple aperture photometry with quantile binning, allowing users to compare ξ(r) across brightness groups without leaving the workflow [@photutils1110; @astropy2022].
...
The figure illustrates a representative diagnostic visualization generated by the pipeline; it is included here to demonstrate the type of artefact produced rather than to support a scientific claim.
